...
foreach($all_agents as $agent){ //Store only agents that are currently online into new array
	...
	if($agent->status != 'offline'){ 
		...
		// format group list that agent is part of
		foreach($current_agent->groups as $key=>$group){
			$group_list .= $group->name . "<br/>";
		}
		$current_agent->group_list = $group_list;
	}
}
...
.agent_name, .agent_login,.agent_last_logout,.agent_status,.agent_last_chat_ended,.agent_group, div.col_title {
	float:left;
	width:200px;
	border: solid 1px;
	padding: 10px;	
...
<div class="container">
	...
	<div class="col_title">Member of Group(s)</div>
...
<?php foreach($agents as $agent){ ?>
	<div id="<?php print $agent->login; ?>">
		...
		<div class="agent_group"><?php print $agent->group_list; ?></div>
